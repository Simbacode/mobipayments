 ...
 * they do things like connection pooling. They also support reading the body of
 * an HTTP response whose status code isn't 200 (OK), which can enable your
...
public class URLConnectionClient implements HttpClient {
	...
	public HttpResponseMessage execute(HttpMessage request,
		...
		for (Map.Entry<String, String> header : addHeaders) {
			...
			headers.append(key).append(": ").append(header.getValue())
					.append(EOL);
		}
		byte[] excerpt = null;
			...
			try {
				if (contentLength != null) {
					((HttpURLConnection) connection)
							.setFixedLengthStreamingMode(Integer
		...
		return new URLConnectionResponse(request, headers.toString(), excerpt,
				connection);
	}
